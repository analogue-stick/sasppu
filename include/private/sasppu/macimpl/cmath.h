...
// SASPPU_HANDLE_CMATH(IDENT, HALF_MAIN_SCREEN, DOUBLE_MAIN_SCREEN,
// HALF_SUB_SCREEN, DOUBLE_SUB_SCREEN, ADD_SUB_SCREEN, SUB_SUB_SCREEN,
// FADE_ENABLE, CMATH_ENABLE)
#ifndef IDENT
...
#define CMATH_HELPER_RECOMBINE_COL(in_r, in_g, in_b, col_out)                  \
  {                                                                            \
    ...
    _in_g &= mask;                                                             \
    _in_b &= mask;                                                             \
    uint16x8_t col_out_unflipped =                                             \
        (_in_r << 1) | (_in_g >> 4) | (_in_b >> 10);                           \
...
static void IDENT(uint16x8_t *const scanline) {
  ...
  do {
    ...
    asm volatile(
        "handle_cmath_and_fade %[sub_ss], %[add_ss], %[ss_double], %[ss_half], "
        "%[ms_double], %[ms_half], %[five], %[ten], %[mask_split], %[one], "
        "%[four], %[fade], %[spill_area]"
        :
        : [sub_ss] "i"(SUB_SUB_SCREEN), [add_ss] "i"(ADD_SUB_SCREEN),
          [ss_double] "i"(DOUBLE_SUB_SCREEN), [ss_half] "i"(HALF_SUB_SCREEN),
          [ms_double] "i"(DOUBLE_MAIN_SCREEN), [ms_half] "i"(HALF_MAIN_SCREEN),
          [five] "r"(&CMATH_TWO_FIVE), [ten] "r"(&CMATH_TWO_TEN),
          [mask_split] "r"(&CMATH_MASK_SPLIT), [one] "r"(&CMATH_ONE),
          [four] "r"(&CMATH_TWO_FOUR),
          [fade] "r"(&SASPPU_cmath_state.screen_fade),
          [spill_area] "r"(&spill_area)
        : "a15");
    ...
    asm volatile(
        "handle_cmath_only %[sub_ss], %[add_ss], %[ss_double], %[ss_half], "
        "%[ms_double], %[ms_half], %[five], %[ten], %[mask_split], %[one], "
        "%[four], %[spill_area]"
        :
        : [sub_ss] "i"(SUB_SUB_SCREEN), [add_ss] "i"(ADD_SUB_SCREEN),
          ...
          [five] "r"(&CMATH_TWO_FIVE), [ten] "r"(&CMATH_TWO_TEN),
          [mask_split] "r"(&CMATH_MASK_SPLIT), [one] "r"(&CMATH_ONE),
          [four] "r"(&CMATH_TWO_FOUR), [spill_area] "r"(&spill_area)
        : "a15");
#elif !CMATH_ENABLE && FADE_ENABLE
    asm volatile inline("                       \n\t \
            ld.qr q0, %[maincol], 0 \n\t \
    ...
    asm volatile(
        "handle_fade_only %[fade], %[one], %[four], %[five], %[ten], "
        "%[mask_split]"
        :
        : [fade] "r"(&SASPPU_cmath_state.screen_fade), [one] "r"(&CMATH_ONE),
          [four] "r"(&CMATH_TWO_FOUR), [five] "r"(&CMATH_TWO_FIVE),
          [ten] "r"(&CMATH_TWO_TEN), [mask_split] "r"(&CMATH_MASK_SPLIT)
        : "a15");
#elif !CMATH_ENABLE && !FADE_ENABLE
    asm volatile inline("                       \n\t \
            ld.qr q0, %[maincol], 0 \n\t \
    ...
    asm volatile("no_cmath_colour_shift %[one], %[eight], %[nine], "
                 "%[mask_low], %[mask_green]"
                 :
                 : [one] "r"(&CMATH_ONE), [eight] "r"(&CMATH_TWO_EIGHT),
                   [nine] "r"(&CMATH_TWO_NINE), [mask_low] "r"(&CMATH_MASK_LOW),
                   [mask_green] "r"(&CMATH_MASK_GREEN)
                 : "a15");
#endif
    asm volatile inline("ee.vst.128.ip q0, %[maincol], -16"
                        : [maincol] "+r"(maincol));
#else
